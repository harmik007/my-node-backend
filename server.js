...
// Dynamically load .env.local variables if the file exists (for local development)
dotenv.config({ path: '.env.local' });

// Fallback to standard .env loading if variables aren't initialized yet
dotenv.config();

const app = express();
const port = process.env.PORT || 3001;

// Global Middlewares - Active across all environments
app.use(cors()); 
app.use(express.json()); 

// Universal database pool configuration using whatever active string is injected
const { Pool } = pg;
const pool = new Pool({
...
// 1. Root Dashboard Screen
app.get('/', (req, res) => {
  res.send(`
    <div style="font-family: sans-serif; padding: 40px; text-align: center; background: #f9f9f9; height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center;">
      <h1 style="color: #333;">🚀 Universal Database Table API Live!</h1>
      <p style="color: #666; font-size: 18px;">Listening for secure data operations from any source origin.</p>
      <div style="margin-top: 20px;">
        <a href="/api/contacts" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; font-weight: bold; font-size: 16px;">
          Fetch Table Entries Data (GET) →
        </a>
...
// 2. GET API Route: Fetches data records from users_contact
app.get('/api/contacts', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, product_name, name, email, phone_number, message, created_at FROM users_contact ORDER BY created_at DESC;');
...
// 3. POST API Route: Insert records matching the layout order
app.post('/api/contact', async (req, res) => {
  const { product_name, name, email, phone_number, message } = req.body;
...
// Starts the server listener automatically across local systems and server environments
app.listen(port, () => {
  console.log(`Universal engine processing operations on port: ${port}`);
});
